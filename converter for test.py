...
# декодирование /uniXXXX, /uXXXX, \uXXXX, &#xXXXX;
_HEX_REPS = [
    r'(?i)/?uni([0-9a-f]{4})',
    r'(?i)/?u([0-9a-f]{4})',
    r'(?i)\\u([0-9a-f]{4})',
...
def is_pdf_textual(pdf_path: str, max_pages_check: int = 3) -> bool:
    """Есть ли реальный текстовый слой (а не картинки) — быстро проверяем первые страницы."""
    try:
...
def pdf_to_image_pdf(original_pdf_path: str, dpi: int = 400) -> str:
    """Рендерим страницы в картинки и собираем временный «image-PDF»."""
...
base_folders = {
    "направления": "тест/направление",
    "доп_данные": "тест/доп данные"
}
    ...
    for file_name in os.listdir(folder_path):
        ...
        try:
            ...
            if file_path.suffix.lower() == ".pdf":
                ...
                if textual:
                    # Есть текстовый слой -> читаем без OCR
                    result = pdf_conv_no_ocr.convert(file_path.as_posix())
                    text = result.document.export_to_markdown()

                    # Если слой «битый» (много /uniXXXX) — переключаемся на OCR
                    if len(re.findall(r'(?i)/uni[0-9a-f]{4}', text)) >= 3:
                        ...
                        # нормализуем имена глифов + lookalikes
                        text = decode_pdf_glyph_names(text)
                        text = fix_lookalikes_if_cyrillic(text)

                    # Таблицы — из исходного файла
                    tables_md = extract_tables_markdown(file_path.as_posix())
                    if tables_md:
                        text += "\n\n" + tables_md

                else:
                    # Нет текстового слоя -> EasyOCR (прямо на PDF) с фолбэком через image-PDF
                    try:
                        result = pdf_conv_easyocr.convert(file_path.as_posix())
                        text = result.document.export_to_markdown()
                    except Exception as e:
        ...
        if code not in docs_by_code:
            docs_by_code[code] = {"код": code, "направления": "", "доп_данные": ""}
